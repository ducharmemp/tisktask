  ...
  defmacro __using__(_opts) do
    quote do
      ...
      def publish(item, event) do
        topic = "#{name_for(item)}:#{event}:#{item.id}"
        Phoenix.PubSub.broadcast(Tisktask.PubSub, topic, item)
        topic = "#{name_for(item)}:#{event}"
        Phoenix.PubSub.broadcast(Tisktask.PubSub, topic, item.id)
        item
      end

      def publish(item, child, event) do
        topic = "#{name_for(item)}:#{event}:#{item.id}"
        Phoenix.PubSub.broadcast(Tisktask.PubSub, topic, {item, child})
        item
      end
